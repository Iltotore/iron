...
For example, most of `RefinedType`'s (see [New types](newtypes.md#runtime-refinement)) methods use a
`RuntimeConstraint`.

It is also recommended to use `RuntimeConstraint` to derive typeclasses, especially when using a `given` with a 
function value.
...
`RuntimeConstraint` is also useful when you need to reuse the same constraint.
Here is a simplified example from `RefinedType`:

```scala
trait RefinedType[A, C]:
