...
object constraints:
  ...
  class DescribedAsConstraint[A, C, Impl <: Constraint[A, C], V <: String](using
      Impl
  ...
  transparent inline given [A, C, Impl <: Constraint[A, C], V <: String](using inline constraint: Impl): Constraint[A, DescribedAs[C, V]] =
  ...
  class NotConstraint[A, C, Impl <: Constraint[A, C]](using Impl)
      extends Constraint[A, Not[C]]:
  ...
  class OrConstraint[A, C1, C2, Impl1 <: Constraint[A, C1], Impl2 <: Constraint[A, C2]](using Impl1, Impl2) extends Constraint[A, Or[C1, C2]]:
    ...
    override inline def message: String = "(" + summonInline[Impl1].message + ") || (" + summonInline[Impl2].message + ")"

  transparent inline given [A, C1, C2, Impl1 <: Constraint[A, C1], Impl2 <: Constraint[A, C2]](using inline left: Impl1, inline right: Impl2): Constraint[A, Or[C1, C2]] = new OrConstraint
  ...
  transparent inline given[A, C1, C2, Impl1 <: Constraint[A, C1], Impl2 <: Constraint[A, C2]](using inline left: Impl1, inline right: Impl2): Constraint[A, And[C1, C2]] = new AndConstraint
  ...
  inline given [A, V <: A]: Constraint[A, StrictEqual[V]] with
    ...
    override inline def message: String =
      "Should strictly equal to " + constValue[ToString[V]]
