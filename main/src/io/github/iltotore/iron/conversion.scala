...
/**
 * Implicitly cast an [[Option]] of an element constrained by `C1` into an [[Option]] of an element constrained by `C2`
 * if `C1` implies `C2`. Unlike [[autoCastIron]], this fires when the element type is already fixed inside the
 * [[Option]] (e.g. through cats' `.some`), where the expected element type can no longer be propagated to the element
 * itself.
 *
 * @param value the option whose element to cast.
 * @param `C1 ==> C2` the evidence that the original constraint `C1` implies `C2`.
 * @tparam A the refined type.
 * @tparam C1 the original constraint.
 * @tparam C2 the target constraint.
 * @return the given option with its element constrained by `C2`.
 */
implicit inline def autoCastIronOption[A, C1, C2](inline value: Option[A :| C1])(using C1 ==> C2): Option[A :| C2] =
  value.asInstanceOf
